...
class Block(nn.Module):
    ...
    """
    def __init__(self, dim, P, drop_path=0., layer_scale_init_value=1e-6, dcls_kernel_size=17, dcls_kernel_count=34):
        super().__init__()
        self.dwconv = cDcls2d(dim, dim, kernel_count=dcls_kernel_count,
                              ...
                              use_implicit_gemm=True, version='v0')
        self.dwconv.P = P
...
class ConvNeXt_dcls(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000,
        ...
        for i in range(4):
            stage = nn.Sequential(
                *[Block(dim=dims[i], P=self.P_stages[i],
...
@register_model
def convnext_dcls_tiny(pretrained=False, **kwargs):
    model = ConvNeXt_dcls(depths=[3, 3, 9, 3], dims=[96, 192, 384, 768],
                          dcls_kernel_size=17, dcls_kernel_count=34, **kwargs)
    if pretrained:
        url = model_urls['convnext_dcls_tiny_1k']
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu", check_hash=True)
        model.load_state_dict(checkpoint["model"])
    return model

@register_model
def convnext_dcls_small(pretrained=False, **kwargs):
    model = ConvNeXt_dcls(depths=[3, 3, 27, 3], dims=[96, 192, 384, 768],
                          dcls_kernel_size=17, dcls_kernel_count=40, **kwargs)
    if pretrained:
        url = model_urls['convnext_dcls_small_1k']
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu")
        model.load_state_dict(checkpoint["model"])
    return model

@register_model
def convnext_dcls_base(pretrained=False, in_22k=False, **kwargs):
    model = ConvNeXt_dcls(depths=[3, 3, 27, 3], dims=[128, 256, 512, 1024],
                          dcls_kernel_size=17, dcls_kernel_count=40, **kwargs)
    if pretrained:
